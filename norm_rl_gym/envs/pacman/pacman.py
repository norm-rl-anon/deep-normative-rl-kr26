...
class GameState:
    ...
    def getLegalActions(self, agentIndex=0):
        ...
        if agentIndex == 0:  # Pacman is moving
            legal = PacmanRules.getLegalActions(self)
            return legal
        else:
    ...
    def __init__(self, prevState=None):
        ...
        if prevState != None:  # Initial state
            ...
            self.killedBlue = prevState.killedBlue
            self.killedOrange = prevState.killedOrange
            self.lastAct = prevState.lastAct
            self.lost = prevState.lost
            self.ateCapsule = prevState.ateCapsule
            # self.train = prevState.train
        else:
            self.data = GameStateData()
            self.killedBlue = False
            self.killedOrange = False
            self.lost = False
            self.ateCapsule = False
            self.lastAct = None
            # self.train = train
    ...
    def __eq__(self, other):
        ...
        """
        return hasattr(other, "data") and self.data.deepCopy() == other.data.deepCopy()
...
class ClassicGameRules:
    ...
    def newGame(
        ...
        game = Game(
            agents, display, self, do_render=do_render, catchExceptions=catchExceptions, monitor=monitor, train=train
        )
    ...
    def collide(state, ghostState, agentIndex):
        if ghostState.isScared():
            ...
            if agentIndex == 1:
                ...
                state.killedBlue = True
            if agentIndex == 2:
                state.data._ghostsEaten2 = state.data._ghostsEaten2 + 1
                state.killedOrange = True

            state.data._eaten[agentIndex] = True
            state.data.agentEatenCnt[agentIndex] = state.data.agentEatenCnt[agentIndex] + 1
...
def readCommand(argv):
    ...
    parser.add_option(
        "--rec",
        dest="rec",
        help=default("Would you like to save a record of tests run? Input file name."),
        default=None,
    )
    parser.add_option("--monitor", dest="monitor", help=default("Specify monitor."), default=None)
...
def runGames(
    ...
    for i in range(numGames):
        ...
        if beQuiet:
            ...
            rules.quiet = False
        if monitor is not None:
            monitorObj = lookup(monitor, globals())()
        else:
            monitorObj = monitor
        game = rules.newGame(layout, pacman, ghosts, gameDisplay, beQuiet, catchExceptions, beQuiet, monitorObj)
        game.run()
        if not beQuiet:
            dic = monitorObj.export()
            dic["Win/Lose"] = game.state.isWin()
            dic["Score"] = game.state.getScore()
            entries.append(dic)
        if not beQuiet:
            games.append(game)
