...
class Layout:
    ...
    def __init__(self, layoutText):
        ...
        self.totalFood = self.food.count()  # originally len(self.food.asList())
        # but we have self.food.count() == len(self.food.asList())
    ...
    def getAllCorners(self):
        return [(1, 1), (1, self.height - 2), (self.width - 2, 1), (self.width - 2, self.height - 2)]
...
def getLayout(name, back=0):
    ...
    if name.endswith(".lay"):
        layout = tryToLoad(path + "/pacman_layouts/" + name)
        if layout == None:
            layout = tryToLoad(name)
    else:
        layout = tryToLoad(path + "/pacman_layouts/" + name + ".lay")
        if layout == None:
