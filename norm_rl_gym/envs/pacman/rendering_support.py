...
def hash_seed(seed: Optional[int] = None, max_bytes: int = 8) -> int:
    ...
    if seed is None:
        seed = 1337  # create_seed(max_bytes=max_bytes)
    hash = hashlib.sha512(str(seed).encode("utf8")).digest()
    return _bigint_from_bytes(hash[:max_bytes])


# gymnasium.utils.seeding.hash_seed = hash_seed
...
# gym_version = tuple(int(x) for x in gymnasium.__version__.split('.'))
#
# __all__ = ['RetroEnv']
...
def get_display(spec):
    ...
    if spec is None:
        ...
        raise Exception("Invalid display specification: {}. (Must be a string like :0 or None.)".format(spec))
